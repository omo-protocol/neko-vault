...
contract UniversalValuerOffchainFixedTest is Test {
    ...
    function setUp() public {
        ...
        valuer.configureSigner(signer2, true, 50);
        valuer.configureSigner(signer3, true, 25);
    ...
    function testUpdateValue() public {
        ...
        signatures[0] = _signValue(STRATEGY_A, value, confidence, nonce, signer1Key);

        vm.expectEmit(true, true, true, true);
        emit ValueUpdated(STRATEGY_A, value, confidence, block.timestamp, true);

        valuer.updateValue(STRATEGY_A, value, confidence, nonce, signatures);
    ...
    function testUpdateValueUnauthorizedSigner() public {
        ...
        bytes[] memory signatures = new bytes[](1);
        signatures[0] = _signValue(STRATEGY_A, 1000e18, 95, 1, unauthorizedKey);

        vm.expectRevert(IUniversalValuerOffchain.InsufficientSignatures.selector);
        valuer.updateValue(STRATEGY_A, 1000e18, 95, 1, signatures);
    }

    function testUpdateValueStaleNonce() public {
        // First update
        bytes[] memory signatures1 = new bytes[](1);
        signatures1[0] = _signValue(STRATEGY_A, 1000e18, 95, 2, signer1Key);
        valuer.updateValue(STRATEGY_A, 1000e18, 95, 2, signatures1);

        // Try with old nonce
        bytes[] memory signatures2 = new bytes[](1);
        signatures2[0] = _signValue(STRATEGY_A, 2000e18, 95, 1, signer1Key);

        vm.expectRevert(IUniversalValuerOffchain.StaleNonce.selector);
        valuer.updateValue(STRATEGY_A, 2000e18, 95, 1, signatures2);
    }
    ...
    function testBatchUpdateEmptyArrays() public {
        ...
        bytes[] memory signatures = new bytes[](1);
        signatures[0] = _signBatchHash(keccak256(abi.encode(strategyIds, values, confidences, 1)), signer1Key);

        // Should succeed with empty arrays
        valuer.batchUpdateValues(strategyIds, values, confidences, 1, signatures);
    }
    ...
    function testPushThresholdUpdate() public {
        ...
        bytes[] memory signatures1 = new bytes[](1);
        signatures1[0] = _signValue(STRATEGY_A, value1, 95, 1, signer1Key);
        valuer.updateValue(STRATEGY_A, value1, 95, 1, signatures1);

        // Try update too soon (should fail)
        vm.warp(block.timestamp + 1 minutes);
        uint256 value2 = 1010e18; // 1% change
        bytes[] memory signatures2 = new bytes[](1);
        signatures2[0] = _signValue(STRATEGY_A, value2, 95, 2, signer1Key);

        vm.expectRevert(IUniversalValuerOffchain.UpdateTooFrequent.selector);
        valuer.updateValue(STRATEGY_A, value2, 95, 2, signatures2);

        // Large change should work even if too soon
        uint256 value3 = 1060e18; // 6% change (> 5% threshold)
        bytes[] memory signatures3 = new bytes[](1);
        signatures3[0] = _signValue(STRATEGY_A, value3, 95, 3, signer1Key);
        valuer.updateValue(STRATEGY_A, value3, 95, 3, signatures3);
    ...
    function testValueStaleness() public {
        uint256 value = 1000e18;
        bytes[] memory signatures = new bytes[](1);
        signatures[0] = _signValue(STRATEGY_A, value, 95, 1, signer1Key);
        valuer.updateValue(STRATEGY_A, value, 95, 1, signatures);
    ...
    function testClearFallbackValue() public {
        ...
        // Update value
        bytes[] memory signatures = new bytes[](1);
        signatures[0] = _signValue(STRATEGY_A, 1000e18, 95, 1, signer1Key);
        valuer.updateValue(STRATEGY_A, 1000e18, 95, 1, signatures);
    ...
    function testLowConfidence() public {
        ...
        bytes[] memory signatures = new bytes[](1);
        signatures[0] = _signValue(STRATEGY_A, value, lowConfidence, 1, signer1Key);
        valuer.updateValue(STRATEGY_A, value, lowConfidence, 1, signatures);
    ...
    function testFallbackValueWithLowConfidence() public {
        ...
        bytes[] memory signatures = new bytes[](1);
        signatures[0] = _signValue(STRATEGY_A, 1000e18, 50, 1, signer1Key);
        valuer.updateValue(STRATEGY_A, 1000e18, 50, 1, signatures);
    ...
    function testWeightedMultiSigCombinedLowWeights() public {
        ...
        signatures[0] = _signValue(STRATEGY_A, 1000e18, 95, 1, signer2Key);
        signatures[1] = _signValue(STRATEGY_A, 1000e18, 95, 1, signer3Key);
    ...
    function testGetTotalValue() public {
        ...
        signaturesA[0] = _signValue(STRATEGY_A, 1000e18, 95, 1, signer1Key);
        valuer.updateValue(STRATEGY_A, 1000e18, 95, 1, signaturesA);

        bytes[] memory signaturesB = new bytes[](1);
        signaturesB[0] = _signValue(STRATEGY_B, 2000e18, 95, 1, signer1Key);
        valuer.updateValue(STRATEGY_B, 2000e18, 95, 1, signaturesB);
    ...
    function testConfigureSigner() public {
        ...
        vm.prank(owner);
        valuer.configureSigner(newSigner, true, 200);
        ...
        bytes[] memory signatures = new bytes[](1);
        signatures[0] = _signValue(STRATEGY_A, 1500e18, 95, 1, newSignerKey);
        valuer.updateValue(STRATEGY_A, 1500e18, 95, 1, signatures);
    }

    function testConfigureSignerUnauthorized() public {
        vm.prank(unauthorized);
        vm.expectRevert();
        valuer.configureSigner(address(0x123), true, 100);
    }
    ...
    function testZeroValue() public {
        ...
        bytes[] memory signatures = new bytes[](1);
        signatures[0] = _signValue(STRATEGY_A, 0, 95, 1, signer1Key);
        valuer.updateValue(STRATEGY_A, 0, 95, 1, signatures);
    ...
    function testMaxUint256Value() public {
        ...
        bytes[] memory signatures = new bytes[](1);
        signatures[0] = _signValue(STRATEGY_A, maxValue, 95, 1, signer1Key);

        valuer.updateValue(STRATEGY_A, maxValue, 95, 1, signatures);
    ...
    function testIsAuthorizedSigner() public {
        ...
        // Disable signer
        vm.prank(owner);
        valuer.configureSigner(signer2, false, 0);
