...
contract YieldAccountingTest is Test {
    ...
    function _allocateAndDeposit(bytes32 _strategyId, uint256 amount) internal {
        ...
        adapter.allocate(
            abi.encode(_strategyId, 0, new IUniversalAdapterEscrow.Call[](0)), amount, bytes4(0), address(0)
        );
    ...
    function testConservativeFallback() public {
        ...
        noValuerAdapter.updateWhitelist(
            address(protocol), bytes4(keccak256("withdraw(uint256)")), true, type(uint256).max
        );
        noValuerAdapter.updateWhitelist(
            address(asset), bytes4(keccak256("approve(address,uint256)")), true, type(uint256).max
        );
        ...
        noValuerAdapter.allocate(
            abi.encode(sid, 0, new IUniversalAdapterEscrow.Call[](0)), 1000e18, bytes4(0), address(0)
        );
        ...
        // User deallocates 500
        vm.prank(address(vault));
        noValuerAdapter.deallocate(
            abi.encode(sid, 0, new IUniversalAdapterEscrow.Call[](0)), 500e18, bytes4(0), address(0)
        );
    ...
    function testCompleteWithdrawal() public {
        ...
        vm.prank(address(vault));
        adapter.deallocate(
            abi.encode(strategyId, 0, new IUniversalAdapterEscrow.Call[](0)), 1500e18, bytes4(0), address(0)
        );
